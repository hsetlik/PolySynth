...
void encoderTurn(uint8_t enc, uint8_t clockwise);
void onClick(uint8_t btn);
void onPressStart(uint8_t btn);
void onPressEnd(uint8_t btn);
void duringPress(uint8_t btn);
/* USER CODE END PFP */
...
uint8_t nextVoiceToUpdate() {
	...
	for (uint8_t i = 0; i < 6; i++) {
		...
		for (uint8_t cv = 0; cv < 7; cv++) {
			if (voiceLevels[voice].currentData[cv]
					!= voiceLevels[voice].prevData[cv])
				return voice;
		}
	}
	return 6;
...
void HAL_SPI_TxCpltCallback(SPI_HandleTypeDef *hspi) {
	if (hspi == &hspi1) { // voices are done
		...
		ILI9341_Unselect();
		// alert the processor
		disp_dma_finished(graphicsProc);
...
void HAL_I2C_TxCpltCallback(I2C_HandleTypeDef *i2c) {
	...
	if (i2c == &hi2c3) { // our DAC is done updating to the current channel
		...
		if (v < 6) { // we have another update to do
			currentDacVoice = v;
			startVoiceUpdate_DMA(i2c, &voiceLevels[v].currentData[0],
					&voiceLevels[v].prevData[0]);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *timer) {
	...
	if (timer == &htim9) {
		...
		// ask the main process for new CV data
		dacLevelsNeeded = 1;
		// if we aren't already mid-transmission, check if we need to transmit anything
		if (dacTransmissionFinished) {
			uint8_t v = nextVoiceToUpdate();
			if (v < 6) {
				dacTransmissionFinished = 0;
				currentDacVoice = v;
				startVoiceUpdate_DMA(&hi2c3, &voiceLevels[v].currentData[0],
						&voiceLevels[v].prevData[0]);
			}
		}
	}
...
void HAL_TIM_PWM_PulseFinishedHalfCpltCallback(TIM_HandleTypeDef *htim) {
	pixel_tx_half_complete(pixelProc, htim);
}

void HAL_TIM_PWM_PulseFinishedCallback(TIM_HandleTypeDef *htim) {
	pixel_tx_complete(pixelProc, htim);
}
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	enc_register_callback(encoderProc, &encoderTurn);
	set_on_click(buttonProc, &onClick);
...
//	}
	...
	HAL_UART_Receive_IT(&huart1, &currentMidiByte, 1);
  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1) {
		...
		if (midiMsgReady) {
			midi_t msg = MIDI_getLatestMessage();
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOC, HP_MODE_Pin|FOLD_FIRST_Pin|GPIO_PIN_3|LED1_Pin
                          |LED2_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, PITCH_RST_Pin|LDAC_Pin|DISP_CS_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : HP_MODE_Pin FOLD_FIRST_Pin PC3 LED1_Pin
                           LED2_Pin */
  GPIO_InitStruct.Pin = HP_MODE_Pin|FOLD_FIRST_Pin|GPIO_PIN_3|LED1_Pin
                          |LED2_Pin;
  ...
  /*Configure GPIO pins : PITCH_RST_Pin LDAC_Pin DISP_CS_Pin */
  GPIO_InitStruct.Pin = PITCH_RST_Pin|LDAC_Pin|DISP_CS_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
void HAL_GPIO_EXTI_Callback(uint16_t pin) {
	switch (pin) {
	case EXP_INTR_A_Pin: // expander 2 port B
		enc_interrupt_sent(encoderProc, EXP_2_ADDR, 1);
		break;
	case EXP_INTR_B_Pin: // expander 2 port A
		enc_interrupt_sent(encoderProc, EXP_2_ADDR, 0);
		break;
	case EXP_INTR_C_Pin: // expander 3 port A
		enc_interrupt_sent(encoderProc, EXP_3_ADDR, 0);
		break;
	default:
		break;
	}
...
// control callback implementations
void encoderTurn(uint8_t enc, uint8_t clockwise) {
	handle_encoder_turn(synthProc, enc, clockwise);
}

void onClick(uint8_t btn) {
	handle_on_click(synthProc, btn);
}

void onPressStart(uint8_t btn) {
	handle_on_press_start(synthProc, btn);
}

void onPressEnd(uint8_t btn) {
	handle_on_press_end(synthProc, btn);
}

void duringPress(uint8_t btn) {
	handle_during_press(synthProc, btn);
}
