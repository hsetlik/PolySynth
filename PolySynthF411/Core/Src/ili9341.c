...
void ILI9341_DrawImage_DMA(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
		uint16_t *data) {
	if ((x >= ILI9341_WIDTH) || (y >= ILI9341_HEIGHT))
		return;
	if ((x + w - 1) >= ILI9341_WIDTH)
	...
	if ((y + h - 1) >= ILI9341_HEIGHT)
		return;
	// pull CS down and set the addres window with blocking SPI
	ILI9341_Select();
	ILI9341_SetAddressWindow(x, y, x + w - 1, y + h - 1);

	// Pull DC up to send pixel data
	HAL_GPIO_WritePin(ILI9341_DC_GPIO_Port, ILI9341_DC_Pin, GPIO_PIN_SET);

	size_t buffSize = w * h * 2;
	HAL_SPI_Transmit_DMA(&ILI9341_SPI_PORT, (uint8_t*)data, buffSize);
